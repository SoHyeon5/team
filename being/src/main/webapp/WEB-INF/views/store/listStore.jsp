...
<!-- 		브랜드 -->
		...
		<p class="brand-font">${storeVO.brand}</p>
		
<!-- 		이름 -->
		<p class="name-font"><%-- <a class="name-a-font" href="read.do?no=${store.num}&pageNo=${storePage.currentPage}"> --%>
		<c:out value="${storeVO.name}"/><!-- </a> --></p>
		
		
<!-- 		할인가 -->
		<p class="dcprice-font">${storeVO.dcprice}</p>
<!-- 		평점 -->
		<p class="avggrade-font">평점${storeVO.avggrade}</p>
<!-- 		무료배송 -->
...
<%--          <a href="list.do?pageNo=${storePage.startPage - 5}">[이전]</a> --%>
<%--          </c:if> --%>
